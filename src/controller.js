...
export function createTextResizerController(config) {
  ...
  let parsedStep = validateStep(rawStep)
 ...
 */
  ...
  let dominantUnit = checkUnitConsistency(originalUnits)

  if (dominantUnit === null) {
    convertAllToPixels(elements, originalTextSizes, originalUnits)
    dominantUnit = 'px'
  }

  // If there is no consistant unit, convert all to pixels.
  if (needForConversion(stepUnit, dominantUnit, parsedStep)) {
    parsedStep = convertStepToUnit(parsedStep, stepUnit, dominantUnit, elements, originalTextSizes)
  }
  ...
  function increase() {
    elements.forEach(element => {
      ...
      const parsedMaxSize = typeof maxSize === 'string'
      ? parseFloat(maxSize) : maxSize
      const clampedSize = Math.min(newSize, parsedMaxSize)

      if (isNaN(clampedSize)) {
        console.warn(`Invalid clampedSize: ${newSize} vs ${maxSize}`)
      }
  ...
  function changeFont(fontFamily) {
    const font = isSerif ? 'sans-serif' : 'serif'
    elements.forEach(element => {
...
function convertAllToPixels(elements, originalTextSizes, originalUnits) {
  elements.forEach(element => {
...
function convertStepToUnit(stepValue, fromUnit, toUnit, elements, originalTextSizes) {
  ...
  if (fromUnit === '_no_unit_') {
    ...
    if (!isAbsolute && !isSmall) {
      ...
      return calculateAverageStepRelativeStep(elements, stepValue, originalTextSizes)
    }
  ...
  if (['px', '%'].includes(fromUnit) && ['em', 'rem', 'vw', 'vh'].includes(toUnit)) {
    return calculateAverageStepRelativeStep(elements, stepValue)
  }
...
function calculateAverageStepRelativeStep(elements, stepValue, originalTextSizes) {
  let totalRelativeStep = 0
  let count = 0

  elements.forEach(element => {
    const originalSize = originalTextSizes.get(element)
    const match = originalSize.match(/^([\d.]+)([a-z%]+)$/)
    ...
    if (match) {
      ...
      if (!isNaN(pxEquivalent) && pxEquivalent > 0) {
        const increasedPixelValue = pxEquivalent + stepValue
        const multiplier = increasedPixelValue / pxEquivalent
        const relativeSizeIncreased = relativeValue * multiplier
        const relativeStep = relativeSizeIncreased - relativeValue

        totalRelativeStep += relativeStep
        count++
      }
    }
  })

  const averageRelativeStep = count > 0 ? totalRelativeStep / count : 1
  return parseFloat(averageRelativeStep.toFixed(2))
}
...
function validateStep(step) {
  ...
  const floatStep = step.match(/^(\d+(\.\d+)?)$/)
  if (floatStep) return parseFloat(floatStep[1])
